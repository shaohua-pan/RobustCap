...
pt_path = os.path.join(config.paths.pw3d_raw_dir, 'kp2d_occ_mp')
...
def detect_mp(debug=True, border=60, vis_mp=True):
    sequences = [x.split('.')[0] for x in os.listdir(os.path.join(data_path, 'imageFiles'))]
    for name in tqdm(sequences):
        for seq in Pw3d_data.pw3d_occluded_sequences:
            if seq in name:
                data = pickle.load(open(os.path.join(data_path, 'sequenceFiles', 'all', name + '.pkl'), 'rb'), encoding='latin1')
                ...
                for p_id in range(num_people):
                    ...
                    for index, im_name in enumerate(im_names):
                        ...
                        if j2d[index, :, 2].mean() < 0.3:
                            uvs.append(None)
                            continue
                        sx, sy, ex, ey = int(max(0, c_x[index] - w[index] // 2)), int(
                            max(0, c_y[index] - h[index] // 2)), int(min(c_x[index] + w[index] // 2, im.shape[1])), int(
                            min(c_y[index] + h[index] // 2, im.shape[0]))
                        im = im[sy:ey, sx:ex]
                        with mp_pose.Pose(
                                model_complexity=1,
                            ...
                            if debug and vis_mp:
                                ...
                                if uv is not None:
                                    for kp in uv:
                                        frame = cv2.circle(frame, (int(kp[0]), int(kp[1])), radius=5,
